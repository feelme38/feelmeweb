...
typedef UpdateCustomerCallback = void Function(UpdateCustomerBody);

class UpdateCustomerDialog extends StatefulWidget {
  const UpdateCustomerDialog(
      {super.key, required this.createUserCallback, required this.customer});

  final CustomerResponse customer;
  final UpdateCustomerCallback createUserCallback;

  @override
  State<UpdateCustomerDialog> createState() => _AddAddressDialogState();
}

class _AddAddressDialogState extends State<UpdateCustomerDialog> {
  final phoneController = TextEditingController();
  final nameController = TextEditingController();
  final ownerController = TextEditingController();
  final listRegions =
      List<RegionResponse>.from(getIt<RegionsRepository>().regions);
  late var selectedRegion = listRegions.first;

  @override
  void initState() {
    phoneController.text = widget.customer.phone.orDash();
    nameController.text = widget.customer.name.orDash();
    ownerController.text = widget.customer.ownerName.orDash();
    selectedRegion = listRegions.first;
    super.initState();
  }
